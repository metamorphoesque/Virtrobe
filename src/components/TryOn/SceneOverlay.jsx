...
const ActionButton = ({ label, onClick, isLoggedIn, disabledReason, onOpenAuth, dark = false }) => {
  const handleClick = () => (!isLoggedIn ? onOpenAuth?.() : onClick?.());
  const btn = (
    <button
      onClick={handleClick}
      className={`
  ...
  return !isLoggedIn ? <Tooltip text={disabledReason}>{btn}</Tooltip> : btn;
};

const SlotBox = ({ label, garmentData, onClear, accentColor }) => {
  const filled = !!garmentData;
  return (
    <div className={`relative rounded-2xl border transition-all duration-200 backdrop-blur-md px-3 py-2.5
      ${filled ? 'bg-white/15 border-white/30' : 'bg-white/5 border-white/15 border-dashed'}`}>
      ...
      {filled ? (
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-white font-medium truncate max-w-[56px]">{garmentData.name ?? label}</span>
          <button onClick={onClear} className="ml-auto text-white/40 hover:text-white/80 transition-colors flex-shrink-0">
...
const AddLayerButton = () => (
  <Tooltip text="Multi-layer outfits — coming with Premium">
    <button disabled className="flex items-center justify-center gap-1.5 w-full rounded-2xl border border-dashed border-white/15 bg-white/5 backdrop-blur-md px-3 py-2 text-[10px] text-white/25 cursor-not-allowed select-none">
      <Plus className="w-3 h-3" />Add layer<Lock className="w-2.5 h-2.5 ml-0.5 opacity-60" />
    </button>
  </Tooltip>
);

// ---------------------------------------------------------------------------
// View angle buttons — Front / Side / Back
// ---------------------------------------------------------------------------
const ViewControls = ({ activeView, onViewChange, visible }) => {
  const views = [
  ...
  return (
    <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-30 transition-all duration-500
      ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
      <div className="flex items-center gap-1 bg-black/50 backdrop-blur-md rounded-2xl p-1 border border-white/10 shadow-lg">
        {views.map(({ key, label }) => (
          <button
            key={key}
            onClick={() => onViewChange(key)}
            className={`px-4 py-1.5 rounded-xl text-[11px] font-medium transition-all duration-200 select-none
              ${activeView === key
  ...
  return (
    <>
      ...
      <ViewControls
        ...
        visible={mannequinSelected}
      />

      {/* LEFT — Layer panel */}
      <div className={`absolute left-4 bottom-16 z-30 flex flex-col gap-2 transition-all duration-500 ease-out
        ${mannequinSelected ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4 pointer-events-none'}`}>
        <AddLayerButton />
      ...
      <div className={`absolute right-4 bottom-16 z-30 flex flex-col gap-2 w-40 transition-all duration-500 ease-out
        ${mannequinSelected ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4 pointer-events-none'}`}>
        <ActionButton label="Save Outfit" onClick={onSave}
          isLoggedIn={isLoggedIn} disabledReason="Sign in to save outfits" onOpenAuth={onOpenAuth} />
        <ActionButton label="Post on Virtrobe" onClick={onPost}
          isLoggedIn={isLoggedIn} disabledReason="Sign in to post your look" onOpenAuth={onOpenAuth} dark />
      </div>
    </>
  );
};
