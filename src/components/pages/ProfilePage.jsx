...
import PrivateOutfitModal from '../ui/PrivateOutfitModal';
...
const ProfilePage = ({ currentUser, onOpenAuth, initialProfile, onProfileUpdate }) => {
  ...
  const [privateOutfit, setPrivateOutfit] = useState(null);
  ...
  const handleClickOutfit = useCallback((outfit) => {
    if (outfit.submission_id) {
      setOutfitModalId(outfit.submission_id);
    } else {
      // Private outfit — open the private detail modal
      setPrivateOutfit(outfit);
    }
  }, []);
  ...
  return (
    <>
      ...
      {privateOutfit && (
        <PrivateOutfitModal outfit={privateOutfit} onClose={() => setPrivateOutfit(null)} />
      )}
