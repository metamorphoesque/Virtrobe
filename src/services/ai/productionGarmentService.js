...
// Replaces hybridGarmentGenerator.js
// ============================================
...
class ProductionGarmentService {
  ...
  async generate(imageFile, userMeasurements) {
    ...
    try {
      ...
      // (Same as your old hybridGarmentGenerator)
      // ============================================
      ...
      const garmentData = {
        ...
        type: classification.type,     // 'tshirt', 'dress', etc.
        classification,                // Full classification data
        method: 'tensorflow-bodypix',  // Changed from 'hybrid-template-modified'
        hasShapeKeys: false,
        dominantColor,
        depthMap: null,               // Not needed anymore
        needsPhysics: true,
  ...
  generateMesh(boundaries, textureUrl, measurements, garmentType) {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      // Different curves for different garments
      switch (garmentType) {
        case 'tshirt':
        case 'shirt':
          // Torso curve with shoulder emphasis
          const shoulderCurve = v > 0.7 ? (v - 0.7) * 30 : 0;
          z = Math.sin(u * Math.PI) * (12 + shoulderCurve) * (1 - v * 0.5);
          break;

        case 'tanktop':
          z = Math.sin(u * Math.PI) * 10 * (1 - v * 0.6);
          break;

        case 'dress':
          // Flared bottom
          const flare = v < 0.4 ? 0 : (v - 0.4) * 2;
          z = Math.sin(u * Math.PI) * (8 + flare * 10) * (1 - v * 0.3);
          break;

        case 'pants':
        case 'shorts':
          // Leg separation
          const legGap = Math.abs(u - 0.5) * 25 * v;
          z = Math.sin(u * Math.PI) * 6 + legGap;
          break;

        case 'skirt':
          // A-line skirt
          const skirtFlare = v * v * 15;
          z = Math.sin(u * Math.PI) * (5 + skirtFlare);
          break;

        default:
          z = Math.sin(u * Math.PI) * 10 * (1 - v * 0.5);
      }
    ...
    // Scale to measurements (same as your old system)
   ...
   * (EXACT SAME as your hybridGarmentGenerator)
   */
